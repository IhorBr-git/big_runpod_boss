...
# ------------------------------------------------------------------------------
# configure_comfyui_run_gpu — ensure run_gpu.sh passes --listen + CORS for RunPod
# ------------------------------------------------------------------------------
...
# A cold `import torch` is slow, so probing the stack with the old 5+ separate
# `python -c "import torch..."` calls wasted several minutes on a healthy restart.
# This imports torch exactly ONCE and prints two space-separated tokens:
...
# gradio (its system-site copy is often too old); numpy/scikit-image/mediapipe are
# pinned because ControlNet's installer otherwise pulls an ABI-mismatched
# numpy/skimage combo that crashes A1111 at `from skimage import exposure`. The
# constraints file is (re)written every boot because ControlNet's own pip installs
# during webui.sh read it via PIP_CONSTRAINT. A single python probe replaces the
# old unconditional --force-reinstall, which ran on every restart even when
# nothing had changed.
# ------------------------------------------------------------------------------
ensure_a1111_deps() {
